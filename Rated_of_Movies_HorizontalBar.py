...
def rate_movies():
    """ Create Rated of Movies horizontalbar_graph of movies in year 2000-2016 and Load Dataset title and rate"""
    for year in range(2000, 2017):
        ...
        graph = pygal.HorizontalBar(x_title="Movie Name & Rated")
        graph.title = "Rated of Movies in year %i Dataset" % year
        for i in range(len(title)):
            graph.add(title[i], rate[i])
        graph.render_to_file("Graph_Export/Rated_Movies/HorizontalBar/Rated_of_Movies_%i.svg" % year)
